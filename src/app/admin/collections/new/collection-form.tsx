...
export function CollectionForm({ action, initialPage }: { action: (formData: FormData) => void, initialPage?: string }) {
  const [name, setName] = useState("");
  const [slug, setSlug] = useState("");
  
  // Auto-detect mode based on page grouping
  const defaultMode = (initialPage === 'profil' || initialPage === 'layanan' || initialPage === 'beranda') ? 'singleton' : 'list';
  const defaultPresentation = (initialPage === 'profil') ? 'key_value' : (initialPage === 'layanan' ? 'cards' : 'list');
    ...
    <Card className="animate-fade-in-up border border-[#6B8E7B]/10 shadow-sm rounded-2xl overflow-hidden bg-white">
      <CardHeader className="border-b border-[#6B8E7B]/10 bg-gradient-to-r from-[#FAF9F6] to-white py-5 px-6">
        <CardTitle className="flex items-center gap-3 text-lg font-black text-[#334155]">
          ...
          Detail Komponen {initialPage ? `untuk Halaman ${initialPage.replace('-', ' ')}` : ''}
        </CardTitle>
        <CardDescription className="text-[#64748B] font-medium ml-13">
          Isi formulir ini untuk membuat struktur komponen data baru.
        </CardDescription>
      </CardHeader>
        ...
        <form action={action} className="space-y-7">
          ...
          <div className="space-y-2">
            ...
            <Input 
              id="name" 
              name="name" 
              placeholder="Misal: Visi Misi, Daftar Beasiswa, Surat Keterangan Usaha" 
              required 
              value={name}
              onChange={handleNameChange}
              className="h-11 border-slate-200 focus-visible:ring-[#6B8E7B]/20 focus-visible:border-[#6B8E7B] rounded-xl shadow-sm"
            />
            <p className="text-xs text-[#94A3B8] font-medium">Nama yang mudah dibaca oleh manusia, tampil sebagai judul bagian.</p>
          </div>
          ...
          <div className="space-y-2">
            ...
            <Input 
              ...
              value={slug}
            />
            <p className="text-xs text-[#94A3B8] font-medium">Dihasilkan secara otomatis dari Nama Komponen (hanya untuk ID internal sistem).</p>
          </div>
          ...
          <div className="grid sm:grid-cols-2 gap-6 bg-[#FAF9F6] p-6 rounded-2xl border border-[#6B8E7B]/10">
            ...
            {!initialPage && (
              <div className="sm:col-span-2 space-y-2">
                ...
                <select id="page" name="page" className="flex h-11 w-full rounded-xl border border-slate-200 bg-white px-3.5 py-2.5 text-sm shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#6B8E7B]/20 focus-visible:border-[#6B8E7B] transition-all">
                ...
                <p className="text-xs text-[#94A3B8] font-medium">Pilih di rute mana komponen ini akan ditampilkan.</p>
              </div>
            )}
            
            {initialPage && (
              <input type="hidden" name="page" value={initialPage} />
            )}

            <input type="hidden" name="mode" value={defaultMode} />
            <input type="hidden" name="presentationType" value={defaultPresentation} />
            
          </div>

          <div className="flex justify-end pt-5 border-t border-[#6B8E7B]/10">
            <Button type="submit" size="lg" className="bg-[#6B8E7B] hover:bg-[#557162] shadow-sm rounded-xl h-11 px-6">
              <Save className="mr-2 h-4 w-4" />
              Simpan &amp; Lanjut ke Fields
            </Button>
          </div>
